...
The firmware is based on existing open-source alternative firmware work for Quansheng handheld radios, including:

- [DualTachyon/uv-k5-firmware](https://github.com/DualTachyon/uv-k5-firmware)
...
- [rebezhir/openquack](https://github.com/rebezhir/openquack)
...
It has been adapted and simplified for a receive-only, accessible ARDF use case.
...
This firmware adds the features needed to use the radio as an amateur radio direction finding (ARDF) receiver on the 2 m and 70 cm bands.

Please treat this as a proof of concept. This is experimental software, there is absolutely no warranty, and you use it entirely at your own risk.

Even with its limitations, this is one of the quickest and most affordable ways to build a usable 2 m / 70 cm ARDF receiver.
...
* [Talking Peiler Controls](#talking-peiler-controls)
* [Manual](#manual)
...
## Talking Peiler Controls

This project is currently being reduced towards a blind-friendly RX-only ARDF receiver ("talking peiler"). The controls below describe the current simplified operating concept in this repository.

### Main Screen

Useful controls currently kept on the main screen:

* `0-9`: direct channel or frequency entry
* `UP/DOWN`: next channel/frequency, or manual gain in ARDF simple mode
* `MENU short`: open menu
* `MENU long`: short spoken key help
* `EXIT short`: compact spoken status
* `EXIT long`: spoken "where am I?" query
* `F`: reduced function layer
* `F` + `2`: switch A/B
* `F` + `3`: switch VFO/MR

### Spoken Feedback

Current audio guidance on the main screen:

* direct entry announces whether channel or frequency entry is active
* completed direct entry announces the selected channel or frequency
* `UP/DOWN` announces the new channel/frequency or ARDF gain
* `EXIT short` announces channel/frequency plus whether `F` is active
* `EXIT long` announces the current operating state
* `MENU long` plays the compact key help
* `F` announces `function`, leaving `F` announces `cancel`

### ARDF / DF Simple

Important controls in ARDF simple mode:

* `PTT short`: snapshot / field-strength beeper
* `UP/DOWN`: manual gain
* `EXIT short`: active fox plus remaining time
* `EXIT long`: fox frequency plus modulation code
* `MENU long`: ARDF-specific audio help

### Deliberately Disabled

Blocked on purpose to keep the radio simpler and safer for blind ARDF use:

* DTMF entry on `*`
* CTCSS/DCS scan on `F` + `*`
* legacy function actions on `F` + `4` to `F` + `9`
* transmit should remain blocked by the RX-only build settings
...
1. Turn the power knob at least 90Â° to switch the device on. 
...
Turn the power knob at least 90Â° to switch the device on. 
