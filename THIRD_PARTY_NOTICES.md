# Third-Party Notices

This project is based on and informed by open-source alternative firmware work for Quansheng handheld radios.

Referenced upstream projects:

- Dennis DL9CAT
  https://github.com/reald/uv-k5-firmware-custom
- DualTachyon/uv-k5-firmware - Apache-2.0
  https://github.com/DualTachyon/uv-k5-firmware
- OneOfEleven/uv-k5-firmware-custom - Apache-2.0
  https://github.com/OneOfEleven/uv-k5-firmware-custom
- egzumer/uv-k5-firmware-custom - Apache-2.0
  https://github.com/egzumer/uv-k5-firmware-custom
- fagci/uv-k5-firmware-fagci-mod - Apache-2.0
  https://github.com/fagci/uv-k5-firmware-fagci-mod
- rebezhir/openquack - Apache-2.0
  https://github.com/rebezhir/openquack
- armel/uv-k1-k5v3-firmware-custom - Apache-2.0
  https://github.com/armel/uv-k1-k5v3-firmware-custom
- Tunas1337/UV-K5-Modded-Firmwares - BSD-2-Clause
  https://github.com/Tunas1337/UV-K5-Modded-Firmwares

Please retain original copyright and license notices in files derived from upstream sources.
